...
import type { ParseResult, ParsedPriceLine } from "../types/parser";
...
export function parseFreePriceText(rawText: string): ParseResult {
  const normalized = normalizeText(rawText);
  const tokens = tokenize(normalized);
  const aliasMap = new Map(PARSER_ALIASES.map((entry) => [entry.alias, entry.caliberId]));
  const results = new Map<string, ParsedPriceLine>();
