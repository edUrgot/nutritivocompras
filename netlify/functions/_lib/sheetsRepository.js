...
export class SheetsRepository {
    ...
    async withSheetsOperation(operation, sheetName, range, execute, metadata) {
    ...
    async getBootstrapData() {
        ...
        return {
            suppliers: providers.length > 0
                ? dedupeByKey(providers.map((row) => ({
            ...
            calibers: CALIBERS,
            parserAliases: parserAliases.length > 0
    ...
    async savePurchase(payload, purchaseId) {
    ...
    async upsertLatestPrices(summary) {
        ...
        for (const line of summary.lines) {
            ...
            const existingIndex = memoryStore.latestPrices.findIndex((item) => item.supplierId === record.supplierId && item.caliberId === record.caliberId);
            if (existingIndex >= 0) {
    ...
    async getNextDocumentNumber(purchaseDate) {
    ...
    async getLatestPrices(supplierId) {
        const sheetRows = await this.readSheetObjects(this.env.sheets.latestPrices, "lenient");
        if (sheetRows.length > 0) {
            const latestPrices = dedupeByKey(sheetRows.map((item) => this.mapLatestPriceRecord(item)), (item) => `${item.supplierId}-${item.caliberId}`).filter((item) => (supplierId ? item.supplierId === supplierId : true));
            return { latestPrices };
        }
        return {
    ...
    async getCompareData() {
        ...
        const cheapestByCaliber = Object.values(matrix.reduce((acc, item) => {
    ...
    async getDashboardData() {
        ...
        const supplierShare = Object.entries(purchases.reduce((acc, purchase) => {
            acc[purchase.header.supplierName] = (acc[purchase.header.supplierName] || 0) + purchase.grandTotal;
            return acc;
        }, {}))
            .map(([label, value]) => ({ label, value }))
            .sort((a, b) => b.value - a.value);
        const caliberShare = Object.entries(purchases.flatMap((purchase) => purchase.lines).reduce((acc, line) => {
    ...
    async appendRow(sheetName, values) {
        ...
        await this.withSheetsOperation("append", sheetName, `${sheetName}!A1`, () => sheets.spreadsheets.values.append({
    ...
    async upsertByKey(sheetName, keyHeader, keyValue, values) {
        const sheets = await getSheetsClient(this.env);
        if (!sheets || !this.env.spreadsheetId) {
        ...
        await this.withSheetsOperation("update", sheetName, `${sheetName}!A${rowIndex + 1}`, () => sheets.spreadsheets.values.update({
            spreadsheetId: this.env.spreadsheetId,
    ...
    async upsertByCompositeKey(sheetName, keyHeaders, keyValues, values) {
        const sheets = await getSheetsClient(this.env);
        if (!sheets || !this.env.spreadsheetId) {
        ...
        const rowIndex = rows.findIndex((row, index) => index > 0 && keyIndexes.every((keyIndex, position) => String(row[keyIndex] ?? "") === keyValues[position]));
        if (rowIndex === -1) {
            await this.appendRow(sheetName, values);
            return;
        }
    ...
    async readSheetObjects(sheetName, mode) {
    ...
    async readSheetRows(sheetName, mode) {
        const sheets = await getSheetsClient(this.env);
        if (!sheets || !this.env.spreadsheetId) {
            if (mode === "lenient") {
                return [];
        ...
        try {
            const response = await this.withSheetsOperation("get", sheetName, `${sheetName}!A:Z`, () => sheets.spreadsheets.values.get({
                spreadsheetId: this.env.spreadsheetId,
                range: `${sheetName}!A:Z`
            }));
    ...
    async ensurePurchaseSheetsReady() {
    ...
    async ensureSheetHeaders(sheetName, expectedHeaders) {
        const rows = await this.readSheetRows(sheetName, "strict");
        if (rows.length === 0) {
